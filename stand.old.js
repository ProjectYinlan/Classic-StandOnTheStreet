...
const avatarSize = 36;
const avatarLineHeight = 22;
const cardWidth = 400;
const avatarInlineCount = 5;
const cardAvatarMargin = parseInt((cardWidth - avatarSize * avatarInlineCount - cardPadding * 2) / (avatarInlineCount - 1));
const cardLineHeight = avatarSize + avatarLineHeight;
...
async function genStandCard(dataObj) {
    ...
    for (let [index, item] of Object.entries(dataObj)) {
        ...
        compositeList.push({
            ...
            left: parseInt(cardPadding + (index % avatarInlineCount) * (cardAvatarMargin + avatarSize))
        })
    }
...
async function genStandAvatarItem(qq, score) {
    ...
    const avatarItem = await sharp({
        create: {
            width: avatarSize,
            height: avatarSize + avatarLineHeight,
            channels: 4,
            background: {
                r: 255,
                g: 255,
        ...
        .composite([
            ...
            },
            {
                ...
                top: avatarSize + 4,
                left: Math.ceil((avatarSize - scoreText.width) / 2)
            }
        ])
        .png()
        .toBuffer()
